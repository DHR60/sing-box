...
GITHUB_UPDATE_CHECKER_FILE = NEW_GITHUB_PATH / "vendor/GitHubUpdateChecker.kt"
...
PACKAGE_PATH_LIST_KEEP = [
    ...
    Path("android/app/src/main/assets/xposed_init"),
]
...
def rewrite_ref1nd_fork_files():
    if GITHUB_UPDATE_CHECKER_FILE.is_file():
        content = GITHUB_UPDATE_CHECKER_FILE.read_text(encoding="utf-8")
        content = re.sub(
            r'(private const val RELEASES_URL\s*=\s*")[^"]+("),?',
            r"\1https://api.github.com/repos/DHR60/sing-box/releases\2",
            content,
        )
        GITHUB_UPDATE_CHECKER_FILE.write_text(content, encoding="utf-8")
    else:
        print(f"Warning: Path does not exist: {GITHUB_UPDATE_CHECKER_FILE}")

    content = BUILD_GRADLE_FILE.read_text(encoding="utf-8")
    content = re.sub(
        r'(\bapplicationId\s*=\s*")[^"]+("),?',
        r"\1com.clearpath.sfa\2",
        content,
        count=1,
    )
