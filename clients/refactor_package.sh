...
echo "=============================="
echo "步骤13: 配置 ProGuard 规则以支持 Go Mobile..."
...
# 更新 build.gradle 中的 shrinkResources 配置
BUILD_GRADLE_FILE="android/app/build.gradle"
if [ -f "$BUILD_GRADLE_FILE" ]; then
  echo "更新 build.gradle 中的资源压缩配置..."
  
  # 查找并替换 shrinkResources 配置
  if grep -q "shrinkResources" "$BUILD_GRADLE_FILE"; then
    # 替换现有的 shrinkResources 配置为 true
    sed -i 's/shrinkResources[[:space:]]*=[[:space:]]*false/shrinkResources = true/g' "$BUILD_GRADLE_FILE"
    sed -i 's/shrinkResources[[:space:]]*=[[:space:]]*true/shrinkResources = true/g' "$BUILD_GRADLE_FILE"
    echo "✓ shrinkResources 已设置为 true"
  else
    # 如果没有找到 shrinkResources，尝试在 release 构建类型中添加
    if grep -A 10 "buildTypes" "$BUILD_GRADLE_FILE" | grep -A 5 "release" | grep -q "minifyEnabled"; then
      # 在 minifyEnabled 行后添加 shrinkResources
      sed -i '/minifyEnabled/a\            shrinkResources = true' "$BUILD_GRADLE_FILE"
      echo "✓ 已在 release 构建类型中添加 shrinkResources = true"
    else
      echo "警告: 无法自动添加 shrinkResources 配置，请手动检查 build.gradle"
    fi
  fi
